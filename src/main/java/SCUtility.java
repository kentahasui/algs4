    ...
    public static Picture doubleToPicture(double[][] grayValues) {
        ...
        for (int col = 0; col < width; col++) {
            for (int row = 0; row < height; row++) {
                ...
                if (normalizedGrayValue >= 1.0) normalizedGrayValue = 1.0;
                picture.set(col, row, new Color((float) normalizedGrayValue, (float) normalizedGrayValue, (float) normalizedGrayValue));
            }
        }
